...
const FeaturedProducts = () => {
  ...
  useEffect(() => {
    const fetchFeaturedProducts = async () => {
      try {
        const url = `${import.meta.env.VITE_API_URI}/products/featured`;

        console.log("Calling featured products API:", url);

        const response = await axios.get(url);

        console.log("Featured API full response:", response);
        console.log("Featured API data:", response.data);
        ...
        console.log("Parsed featured products:", data);
...
{products.slice(0, 8).map((product) => {            // ✅ SAME IMAGE LOGIC AS BEST SELLING
