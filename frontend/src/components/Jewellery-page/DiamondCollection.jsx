  ...
  const fetchDiamondProducts = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_URI}/products/diamond?${query.join("&")}`
      );

      setProducts(res.data);
      setLoading(false);
    } catch (err) {
      console.error("❌ Error fetching diamond products:", err);
          ...
          >
            ...
            <div className="d-flex flex-row align-items-center gap-2">
              ...
              <div className="d-flex flex-row gap-3 text-gray-600">
                {["Earrings", "Rings", "Necklaces"].map((cat) => (
        ...
        <section className="d-flex flex-row flex-wrap justify-start gap-4 p-4 m-5">
          {products.map((prod) => (
            ...
            >
              ...
              <div className="mt-2 ">
                <h3 className="mt-2 text-gray-800">{prod.name}</h3>
                <p className=" text-gray-500 my-2">
                  Net: {prod.netWeight}g | Gross: {prod.grossWeight}g
                </p>

                <div className="mt-1 text-pink-700 font-bold text-md">
                  ₹{prod.price}{" "}
                  {prod.discount > 0 && (
